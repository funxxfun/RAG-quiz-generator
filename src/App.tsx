...
- 日本語で作成`;
  ...
  const submitAnswer = (ans: string) => {
    ...
    const ok =
      ans.trim().toLowerCase() === q.a.toLowerCase() ||
  ...
  const [c1, c2] = cur
    ? TYPE_COLOR[cur.type] || ["#4FACFE", "#00F2FE"]
    : ["#4FACFE", "#00F2FE"];
    ...
    >
      <div style={{ textAlign: "center", marginBottom: 20 }}>
        ...
        <h1
          style={{
            ...
            fontWeight: 900,
            margin: "4px 0 2px",
            background: "linear-gradient(90deg,#4FACFE,#43E97B,#FFB347)",
      ...
      {phase === "home" && (
        ...
        >
          ...
          >
            ...
            <div style={{ fontSize: 11, color: "#6B8899", marginBottom: 10 }}>
              ブラウザで対象ページを開き、本文を選択して{" "}
              <kbd
                style={{
                  background: "#0F1E2A",
                  padding: "1px 6px",
            ...
            >
              <span style={{ fontSize: 11, color: "#6B8899" }}>
                {manualText.length} 文字
              </span>
              <button
                ...
                style={{
                  ...
                  fontWeight: 700,
                  fontSize: 13,
                  cursor: manualText.trim().length < 50 ? "default" : "pointer",
                }}
              >
                追加する
              </button>
            ...
            {status && (
              <div
                style={{
                  marginTop: 8,
                  fontSize: 11,
                  color: status.startsWith("✅") ? "#43E97B" : "#FF6B9D",
                }}
              >
                {status}
              </div>
              ...
              {sources.map((s, i) => (
                ...
                >
                  ...
                  <div style={{ color: "#6B8899" }}>
                    {s.url.slice(0, 50)}… ｜ {s.chars.toLocaleString()}文字
                  </div>
                </div>
              ))}
            </div>
                ...
                {[5, 10, 15, 20].map((n) => (
                  <button
                    ...
                    style={{
                      ...
                      fontSize: 12,
                      cursor: "pointer",
                      background: count === n ? "#FFB347" : "#1e3a4a",
                      color: count === n ? "#fff" : "#6B8899",
                    }}
                  >
                    {n}問
                  </button>
              ...
              <button
                ...
                style={{
                  ...
                  background: generating
                    ? "#1e3a4a"
                    : "linear-gradient(90deg,#FFB347,#f953c6)",
                  color: "#fff",
                  fontWeight: 800,
                  fontSize: 14,
          ...
          {quizzes.length > 0 && (
            ...
            >
              ...
              >
                {quizzes.length}問生成済み ｜ まるばつ{" "}
                {quizzes.filter((q) => q.type === "まるばつ").length}問 ／ 4択{" "}
                {quizzes.filter((q) => q.type === "4択").length}問 ／ 穴埋め{" "}
                {quizzes.filter((q) => q.type === "穴埋め").length}問
                {answered > 0 && (
                  <span style={{ color: "#43E97B" }}>
                    　｜　前回スコア {score}/{answered}
                  </span>
                )}
              </div>
          ...
          {sources.length === 0 && (
            ...
            >
              <div
                style={{ color: "#4FACFE", fontWeight: 700, marginBottom: 6 }}
              >
                📖 使い方
              </div>
      ...
      {phase === "quiz" && cur && (
        <div style={{ width: "100%", maxWidth: 460 }}>
          ...
          >
            <span style={{ fontSize: 12, color: "#6B8899" }}>
              {(currentIdx ?? 0) + 1} / {quizzes.length}問
            </span>
            <span style={{ fontSize: 12, color: "#43E97B" }}>
              ✅ {score}正解
            </span>
            <button
              onClick={() => {
            ...
            <div style={{ padding: "20px 20px 18px" }}>
                  ...
                  {["〇", "✗"].map((ch) => (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 12,
                        border: `2px solid ${
                          ch === "〇" ? "#43E97B44" : "#FF6B9D44"
                        }`,
                        background: "#0F1E2A",
                        color: ch === "〇" ? "#43E97B" : "#FF6B9D",
                        fontWeight: 900,
                        fontSize: 28,
                        cursor: "pointer",
                      }}
                      ...
                      onMouseLeave={(e) =>
                        (e.currentTarget.style.background = "#0F1E2A")
                      }
                    >
                      {ch}
                    </button>
              ...
              {result === null && cur.type === "穴埋め" && (
                <div style={{ display: "flex", gap: 8 }}>
                  <input
                    ...
                    onKeyDown={(e) =>
                      e.key === "Enter" && submitAnswer(userAnswer)
                    }
                    placeholder="答えを入力…"
                    style={{
                      flex: 1,
              ...
              {result === "wrong" && (
                ...
                >
                  ...
                  <div style={{ fontSize: 13, color: "#A0BFC0", marginTop: 4 }}>
                    あなた：
                    <strong style={{ color: "#FF6B9D" }}>{userAnswer}</strong>
                  </div>
          ...
          {result !== null && (
            <div style={{ marginTop: 14, textAlign: "center" }}>
              ...
              >
                {currentIdx !== null && currentIdx + 1 >= quizzes.length
                  ? "🏁 結果を見る"
                  : "次の問題 →"}
              </button>
            </div>
